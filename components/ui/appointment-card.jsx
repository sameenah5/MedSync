...
export function AppointmentCard({ appointment, userRole, refetchAppointments }) {
  ...
  // Effects with setTimeout to avoid cascading renders
  useEffect(() => {
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {userRole === "DOCTOR" && <p className="text-sm text-muted-foreground">{otherParty.email}</p>}
                {userRole === "PATIENT" && <p className="text-sm text-muted-foreground">{otherParty.specialty}</p>}
            ...
            <div className="flex flex-col gap-2 self-end md:self-start">
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {canMarkCompleted() && (
                  <Button size="sm" onClick={handleMarkCompleted} disabled={completeLoading} className="bg-emerald-600 hover:bg-emerald-700">
                    {completeLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <> <CheckCircle className="h-4 w-4 mr-1" /> Complete </>}
                  </Button>
                )}
                <Button size="sm" variant="outline" className="border-emerald-900/30" onClick={() => setOpen(true)}>
                  View Details
                </Button>
              </div>
            </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-white font-medium">{userRole === "DOCTOR" ? otherParty.name : `Dr. ${otherParty.name}`}</p>
                  {userRole === "DOCTOR" && <p className="text-muted-foreground text-sm">{otherParty.email}</p>}
                  {userRole === "PATIENT" && <p className="text-muted-foreground text-sm">{otherParty.specialty}</p>}
                </div>
              </div>
            </div>
            ...
            {appointment.patientDescription && (
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-muted-foreground">{userRole === "DOCTOR" ? "Patient Description" : "Your Description"}</h4>
              ...
              {userRole === "DOCTOR" && action === "notes" ? (
                <div className="space-y-3">
                  ...
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button size="sm" onClick={handleSaveNotes} disabled={notesLoading} className="bg-emerald-600 hover:bg-emerald-700">
                      {notesLoading ? (
                        <>
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2">
            <div className="flex gap-2">
              {canMarkCompleted() && (
                <Button onClick={handleMarkCompleted} disabled={completeLoading} className="bg-emerald-600 hover:bg-emerald-700">
                  {completeLoading ? (
                    <>
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                      Completing...
              ...
              {appointment.status === "SCHEDULED" && (
                <Button variant="outline" onClick={handleCancelAppointment} disabled={cancelLoading} className="border-red-900/30 text-red-400 hover:bg-red-900/10 mt-3 sm:mt-0">
                  {cancelLoading ? (
                    <>
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                      Cancelling...
